...
module.exports = class extends Base{
   async getMeasureListAction(){
    ...
    }else if((!think.isEmpty(roleAnwer)&&roleAnwer.length>0&&roleAnweranswer[0].finsh==2)&&!answer){
  ...
  async saveAnswerAction(){
    ...
    if(!think.isEmpty(organizationAnswer)){
      await this.model('answer_role').add({user_id:userId,answerlist:organizationAnswer,organization_id:organizationId,finish:2})
    }
  ...
  async getAnswerMeasureListAction(){
    ...
    return this.json({success:0,data:data})
  }
};
